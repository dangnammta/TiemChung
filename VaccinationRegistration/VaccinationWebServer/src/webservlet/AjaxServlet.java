...
//        System.out.println("AjaxServlet.doGet.action="+method);
        switch (method) {
            ...
            case "getEventStatus":
                ...
                break;
            case "delete_registraton":
                deleteRegistrationInfo(req, mapjson);
                break;
        }
...
//            this.echo(mapjson.toJSONString()+"", resp);
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        switch (method) {
            ...
            case "login":
                ...
                break;
            case "updateRegistrationInfo":
                updateRegistrationInfo(req, mapjson);
                break;
        }
        if (Configuration.isDebug == 1) {
            long endTime = System.currentTimeMillis();
    ...
    private void deleteRegistrationInfo(HttpServletRequest req, JSONObject mapjson) {
        int registrationID = ServletUtil.getIntParameter(req, "registrationID");
        if (registrationID <= 0) {
            mapjson.put("error_code", -1);
            mapjson.put("error_message", "Tham số không hợp lệ!");
        } else {
            TError result = VREventManagerServiceClient.getInstance().removeRegistration(registrationID);
            if (result != null && result.errorCode == TErrorCode.EC_OK.getValue()) {
                mapjson.put("error_code", 0);
                mapjson.put("error_message", "Xóa đăng ký thành công!");
            } else {
    ...
    private void updateRegistrationInfo(HttpServletRequest req, JSONObject mapjson) {
...
//        int type_register = ServletUtil.getIntParameter(req, "type_register");
        ...
        if (nameChild == null || nameChild.isEmpty()
                ...
                || phoneParent == null || phoneParent.isEmpty() //                || email == null || email.isEmpty()
                ) {
            mapjson.put("error_code", -1);
            mapjson.put("error_message", "Tham số không hợp lệ!");
        } else {
...
//            registerInfo.setChildrenCarrierEmail(email);
            registerInfo.setChildrenCarrierName(nameParent.trim());
            registerInfo.setChildrenCarrierPIN(cmtndParent.trim());
            ...
            TError updateResult = VREventManagerServiceClient.getInstance().updateRegistrationInfo(registrationID, registerInfo);
//            TRMAddExtraRegistrationResponse addResp = VRRegistrationManagerServiceClient.getInstance().addExtraRegistration(registerInfo);

            if (updateResult != null && updateResult.errorCode == 0) {
                mapjson.put("error_code", 0);
                mapjson.put("nameChild", nameChild);
                mapjson.put("birthdayChild", birthdayChild);
                mapjson.put("cmtndParent", cmtndParent.trim());
...
//                mapjson.put("injectionOrderNumber", addResp.injectionOrderNumber);
                mapjson.put("injectionTime", injectionTime);
                mapjson.put("dayOfInjection", dayOfInjection);
            } else {
                if (updateResult == null) {
                    mapjson.put("error_code", "-10");
                    mapjson.put("error_message", "Đường truyền Internet gặp sự cố, vui lòng thử lại sau");
                } else {
                    mapjson.put("error_code", updateResult.errorCode);
                    if (updateResult.errorCode == TErrorCode.EC_PIN_REGISTERED.getValue()) {
                    ...
                    } else if (updateResult.errorCode == TErrorCode.EC_REGISTRATION_FULL.getValue()) {
                        mapjson.put("error_message", "Số lượng Vắcxin của đợt tiêm chủng này đã hết, hẹn quý khách đợt tiêm tiếp theo.");
                    } else {
                        mapjson.put("error_message", "Đường truyền Internet gặp sự cố, vui lòng thử lại sau " + updateResult.errorCode);
                    }
...
//                    resp.sendRedirect("/monitor?t=" + sessionMD5);
    ...
    private void updateStatusPepole(HttpServletRequest req, JSONObject mapjson) {
        ...
        if (userNameSession != null && !userNameSession.isEmpty()) {
            ...
            TError result = VREventManagerServiceClient.getInstance().updateInjectionStatus(registrationID, statusInjection);
            if (result != null && result.errorCode == TErrorCode.EC_OK.getValue()) {
                mapjson.put("error_code", 0);
    ...
    private void getListRegistrationPepole(HttpServletRequest req, JSONObject mapjson) {
        ...
        TVMGetRegistrationsResponse result = VREventManagerServiceClient.getInstance().getRegistrations(eventID);
        if (result != null && result.error.errorCode == 0) {
            mapjson.put("error_code", 0);
...
//        String email = ServletUtil.getStringParameter(req, "email");
        ...
        if (nameChild.isEmpty() || birthdayChild.isEmpty()
                || nameParent.isEmpty() || cmtndParent == null || cmtndParent.isEmpty()
                || phoneParent.isEmpty()
                || address.isEmpty() || type_register < 0 || type_register > 1 || cmtndParent.trim().length() <= 0) {
            mapjson.put("error_code", -1);
            mapjson.put("error_message", "Tham số không hợp lệ!");
        } else {
            Calendar cal = Calendar.getInstance();
